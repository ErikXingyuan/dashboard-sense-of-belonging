...
# load data
@st.cache_data
def load_data(file_path, file_version):
    if not os.path.exists(file_path):
        return None
...
# read file
file_version = get_file_version(DEFAULT_FILE)
raw_df = load_data(DEFAULT_FILE, file_version)
...
with st.sidebar:
    ...
    study_program_selected = st.multiselect(
        "Studiengang",
        get_options(df[program_col]) if program_col else [],
        key="filter_program",
    ) if program_col else []

    gender_selected = st.multiselect(
        "Geschlecht",
        get_options(df[gender_col]) if gender_col else [],
        key="filter_gender",
    ) if gender_col else []

    age_selected = st.multiselect(
        "Alter",
        get_options(df[age_col]) if age_col else [],
        key="filter_age",
    ) if age_col else []

    migration_selected = st.multiselect(
        "Migrationshintergrund",
        get_options(df[migration_col]) if migration_col else [],
        key="filter_migration",
    ) if migration_col else []

    graduation_year_selected = st.multiselect(
        "Abschlussjahr",
        get_options(df[year_col]) if year_col else [],
        key="filter_gradyear",
    ) if year_col else []

    work_selected = st.multiselect(
        "Nebenjob",
        get_options(df[work_col]) if work_col else [],
        key="filter_work",
    ) if work_col else []
